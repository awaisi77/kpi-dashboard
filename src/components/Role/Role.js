...
let roleListUrl = config.base_api_url + "roles";
...
class Role extends Component {
    ...
    removeRecord = (dataItem, e) => {
        this.props.dispatch(spinner(true))
        const payload = {
            url: roleListUrl + '/delete?id='+dataItem.id,
            method: 'GET',
        };
        callApi(payload).then(response => {
            console.log(response)
    ...
    render() {
        ...
        let searchFilterContainer = (
            <div className="mb-1 pageHead">
                <div className="row">
                    <div className="col-6"><h1>Role</h1></div>
                    <div className="col-6">
                        <button type="button"
                                onClick={this.insert}
                                className="btn btn-primary filterSearch ml-2 mt-1 float-right"><i
                            className="fal fa-plus"></i> Add Role
                        </button>
                    </div>
                </div>
            </div>)
        ...
        return (
            <Fragment>
                ...
                <div className="container-fluid">
                    ...
                    {dataGrid}
                    {this.state.dataItemInEdit && <RoleFormDialog dataItem={this.state.dataItemInEdit}
                                                                 roles={this.state.data.grid}
